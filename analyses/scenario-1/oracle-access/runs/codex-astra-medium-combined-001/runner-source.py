...
def prompt_path(provider):
    return HERE / 'prompts' / ('buyer-combined.md' if provider == 'combined' else 'buyer.md')
...
def build_prompt(provider):
    ...
    template = prompt_path(provider).read_text(encoding='utf-8')
    return (template + '\n## Shared background\n\n' + public + '\n## Private user brief\n\n' + private
...
def main():
    ...
    manifest = {
        ...
        'prompt_template_sha256': sha(prompt_path(args.provider).read_bytes()),
