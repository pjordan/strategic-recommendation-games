...
def prompt_path(provider, evidence_policy='card-evidence'):
    if evidence_policy not in ('card-evidence', 'reasonable-representation'):
        raise ValueError('Unknown evidence policy')
    name = 'buyer-combined' if provider == 'combined' else 'buyer'
    if evidence_policy == 'reasonable-representation':
        name += '-reasonable'
...
def build_prompt(provider, evidence_policy='card-evidence'):
    ...
    template = prompt_path(provider, evidence_policy).read_text(encoding='utf-8')
    return (template + '\n## Shared background\n\n' + public + '\n## Private user brief\n\n' + private
...
def main():
    ...
    parser.add_argument('--evidence-policy', choices=['card-evidence','reasonable-representation'], default='card-evidence', help='Explicit interpretation of recommendation-card credibility; original policy remains the default for old commands')
    ...
    prompt = build_prompt(args.provider, args.evidence_policy)
    ...
    manifest = {
        ...
        'provider': args.provider, 'scenario_version': '1.1.0', 'evidence_policy': args.evidence_policy,
        ...
        'prompt_template_sha256': sha(prompt_path(args.provider, args.evidence_policy).read_bytes()),
