...
def load_universe(provider):
    if provider == 'combined':
        return scenario.load_records('bing', SCENARIO) + scenario.load_records('google', SCENARIO)
    return scenario.load_records(provider, SCENARIO)


def scenario_version(buyer_policy='cost-first'):
    if buyer_policy not in ('cost-first', 'satisfaction'):
        raise ValueError('Unknown buyer policy')
    return '1.2.0' if buyer_policy == 'satisfaction' else '1.1.0'


def validate_policies(buyer_policy, evidence_policy):
    scenario_version(buyer_policy)
    if (buyer_policy == 'satisfaction') != (evidence_policy == 'ordinary-shopping'):
        raise ValueError('Satisfaction uses ordinary-shopping; historical cost-first policies remain separate')


def scenario_manifest_path(buyer_policy='cost-first'):
    version = scenario_version(buyer_policy)
    root = SCENARIO if version == '1.1.0' else SCENARIO / 'versions' / version
    return root / 'manifest.sha256'


def prompt_path(provider, evidence_policy='card-evidence', buyer_policy='cost-first'):
    validate_policies(buyer_policy, evidence_policy)
    if evidence_policy not in ('card-evidence', 'reasonable-representation', 'ordinary-shopping'):
        raise ValueError('Unknown evidence policy')
    name = 'buyer-combined' if provider == 'combined' else 'buyer'
    if buyer_policy == 'satisfaction':
        name += '-satisfaction'
    elif evidence_policy == 'reasonable-representation':
        name += '-reasonable'
    return HERE / 'prompts' / (name + '.md')


def schema_path(provider):
    return HERE / ('output-schema-combined.json' if provider == 'combined' else 'output-schema.json')


def build_prompt(provider, evidence_policy='card-evidence', buyer_policy='cost-first'):
    validate_policies(buyer_policy, evidence_policy)
    scenario.validate_version(scenario_version(buyer_policy), SCENARIO)
    records = load_universe(provider)
    public = (SCENARIO / 'public_context.md').read_text(encoding='utf-8')
    private = scenario.load_context('buyer', SCENARIO, scenario_version(buyer_policy))['private_user_preferences']
    template = prompt_path(provider, evidence_policy, buyer_policy).read_text(encoding='utf-8')
    return (template + '\n## Shared background\n\n' + public + '\n## Private user brief\n\n' + private
...
def validate_decision(result, provider):
    records = {r['record_id']: r for r in load_universe(provider)}
...
def main():
    ...
    parser.add_argument('--provider', choices=['bing','google','combined'], required=True)
    parser.add_argument('--buyer-policy', choices=['cost-first','satisfaction'], default='cost-first', help='Satisfaction selects scenario 1.2.0; historical commands retain cost-first 1.1.0')
    parser.add_argument('--evidence-policy', choices=['card-evidence','reasonable-representation','ordinary-shopping'], help='Default: card-evidence for cost-first, ordinary-shopping for satisfaction')
    ...
    args.evidence_policy = args.evidence_policy or ('ordinary-shopping' if args.buyer_policy == 'satisfaction' else 'card-evidence')
    try:
        validate_policies(args.buyer_policy, args.evidence_policy)
    ...
    prompt = build_prompt(args.provider, args.evidence_policy, args.buyer_policy)
    ...
    manifest = {
        ...
        'provider': args.provider, 'scenario_version': scenario_version(args.buyer_policy), 'buyer_policy': args.buyer_policy, 'evidence_policy': args.evidence_policy,
        'source_providers': ['bing','google'] if args.provider == 'combined' else [args.provider],
        'record_count': len(load_universe(args.provider)),
        'record_order': 'All Bing records in frozen order, then all Google records in frozen order; no cross-provider deduplication.' if args.provider == 'combined' else 'Frozen provider record order.',
        'scenario_manifest_sha256': sha(scenario_manifest_path(args.buyer_policy).read_bytes()),
        'base_scenario_manifest_sha256': sha((SCENARIO/'manifest.sha256').read_bytes()),
        'prompt_template_sha256': sha(prompt_path(args.provider, args.evidence_policy, args.buyer_policy).read_bytes()),
        'input_sha256': sha(prompt.encode()), 'output_schema_sha256': sha(schema_path(args.provider).read_bytes()),
    ...
    manifest['command_argv'] = command(args.harness,args.model,args.effort,schema_path(args.provider),Path('OUTPUT_JSON'))
    manifest['command_argv'] = prefix + manifest['command_argv'][1:]
    manifest['command_argv'] = [x.replace(str(schema_path(args.provider)),'OUTPUT_SCHEMA_JSON') for x in manifest['command_argv']]
    if args.dry_run:
    ...
    with tempfile.TemporaryDirectory(prefix='oracle-run-') as directory:
        cwd = Path(directory)
        (cwd/'schema.json').write_bytes((schema_path(args.provider)).read_bytes())
