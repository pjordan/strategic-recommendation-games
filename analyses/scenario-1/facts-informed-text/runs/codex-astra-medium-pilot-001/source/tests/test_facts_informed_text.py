"""The facts rerun changes only recommender instructions and keeps replay intact."""
import contextlib
...
HERE = ROOT/'analyses/scenario-1/facts-informed-text'
spec = importlib.util.spec_from_file_location('facts_study', HERE/'run_study.py')
study = importlib.util.module_from_spec(spec); spec.loader.exec_module(study)


class FactsTextChecks(unittest.TestCase):
    def test_schedule_runs_only_five_text_conditions(self):
        ...
        with self.assertRaisesRegex(ValueError, 'text-enabled'):
            study.FactsGame(cases[0], 'list-only')
    ...
    def test_only_recommender_instruction_changes(self):
        for case in study.plan(1, 0):
            old, new = study.engine.Game(case, 'text-enabled'), study.FactsGame(case, 'text-enabled')
            ...
            for role in ('recommender', 'buyer', 'disclosure', 'followup'):
                ...
                if role == 'recommender':
                    added = (HERE/'prompts/facts-use.md').read_text()
    ...
    def test_complete_mock_study_and_archived_replay(self):
        def invoke(role, prompt, args, directory, prefix, version, *, schema_file, **kwargs):
            ...
            if role == 'disclosure': output = fixtures.plan_output()
                ...
                self.assertIn('Explicit consideration of product facts', prompt)
                case = {'providers': [payload['provider']], 'condition': 'informed-recommender', 'display_order': 'bing-first'}
                output = fixtures.rec_output(study.FactsGame(case, 'text-enabled'), payload['provider'])
            directory.mkdir()
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            verifier = study.engine.runtime.load_module('facts_verifier', HERE/'verify_runs.py')
            ...
            self.assertEqual(sum('Explicit consideration of product facts' in p.read_text() for p in stages), 10)
