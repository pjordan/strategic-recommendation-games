"""Verify strategic-disclosure inputs, information boundaries and observed outcomes."""
import argparse
...
def verify(directory):
    ...
    disclosure=json.loads((directory/'disclosure/output.json').read_text())
    selected=game.validate_disclosure(disclosure)
    messages=selected['messages']
    if messages!=json.loads((directory/'disclosures.json').read_text()):
        raise ValueError('Messages differ from selected disclosure plan')
    if game.buyer_state(disclosure)!=json.loads((directory/'buyer-state.json').read_text()):
        raise ValueError('Buyer prior state differs from selected plan')
    ...
    result=game.outcome(recs,buyer,responses,disclosure,meta['display_order'])
    ...
    stages=[(directory/'recommenders'/p,'recommender',game.recommender_prompt(p,messages[p],meta['display_order'])) for p in ('bing','google')]
    stages.append((directory/'disclosure','disclosure',game.disclosure_prompt(meta['display_order'])))
    stages.append((directory/'buyer','buyer',game.buyer_prompt(responses,disclosure,meta['display_order'])))
